...
namespace ReplicaStudio.Viewer.TransverseLayer.Algorithms
{
    public class PathFinderFast : IPathFinder
    {
        ...
        public PathFinderFast(byte[,] grid)
        {
            ...
            mGrid = (byte[,])grid.Clone();
            //Epuration de la grid
            int width = mGrid.GetUpperBound(0);
            int height = mGrid.GetUpperBound(1);
            ...
            mGridX = (ushort)(width + 1);
            mGridY = (ushort)(height + 1);
        ...
        public List<PathFinderNode> FindNearestPoint(Point start, Point end)
        {
            List<PathFinderNode> output = null;
            int left = 1;
            int right = 1;
            int top = 1;
            int bottom = 1;
            ...
            //1 On détermine le chemin le plus court entre les 5 possibilités.
            //Gauche
            while (end.X - left > 1 && mGrid[end.X - left, end.Y] != 1)
                left++;
            while (right < 1919 && end.X + right < 4096 && mGrid[end.X + right, end.Y] != 1)
                right++;
            while (end.Y - top > 1 && mGrid[end.X, end.Y - top] != 1)
                top++;
            while (bottom < 1199 && end.Y + bottom < 4096 && mGrid[end.X, end.Y + bottom] != 1)
                bottom++;
            while (output == null && (leftActive || rightActive || topActive || bottomActive))
            {
                if (leftActive &&
                    (!rightActive || left <= right) &&
                    (!topActive || left <= top) &&
                    (!bottomActive || left <= bottom))
                {
                    output = FindPath(start, new Point(end.X - left, end.Y), false);
                    if (output != null)
                        break;
                    else
                    {
                ...
                if (rightActive &&
                    (!leftActive || right <= left) &&
                    (!topActive || right <= top) &&
                    (!bottomActive || right <= bottom))
                {
                    output = FindPath(start, new Point(end.X + right, end.Y), false);
                    if (output != null)
                        break;
                    else
                    {
                ...
                if (
                    topActive &&
                    (!leftActive || top <= left) &&
                    (!rightActive || top <= right) &&
                    (!bottomActive || top <= bottom))
                {
                    output = FindPath(start, new Point(end.X, end.Y - top), false);
                    if (output != null)
                        break;
                    else
                    {
                ...
                if (bottomActive &&
                    (!rightActive || bottom <= right) &&
                    (!topActive || bottom <= top) &&
                    (!leftActive || bottom <= left))
                {
                    output = FindPath(start, new Point(end.X, end.Y + bottom), false);
                    if (output != null)
                        break;
                    else
                    {
            ...
            //IEnumerator enumerator = Tools.RenderLine(start, end).GetEnumerator();
            IEnumerator enumerator = Tools.RenderLine(end, start).GetEnumerator();
            while (enumerator.MoveNext())
            {
                /*if (mGrid[((Point)enumerator.Current).X, ((Point)enumerator.Current).Y] == 0)
                    break;
                newEndPoint = (Point)enumerator.Current;*/
                if (mGrid[((Point)enumerator.Current).X, ((Point)enumerator.Current).Y] == 1)
                {
                    newEndPoint = (Point)enumerator.Current;
                    break;
            ...
            output = FindPath(start, newEndPoint, false);

            if (output != null)
                return output;

            enumerator = Tools.RenderLine(start, end).GetEnumerator();
            while (enumerator.MoveNext())
            {
                if (mGrid[((Point)enumerator.Current).X, ((Point)enumerator.Current).Y] == 0)
                    break;
            ...
            return FindPath(start, newEndPoint);
        }
        #endregion
